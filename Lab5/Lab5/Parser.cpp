...
Point CParser::ParsePoint(const std::string &input)
{
	...
	coords.push_back(coord);
	Point point{ CLongNumber(coords[0]), CLongNumber(coords[1]) };

	return point;
}

CLongNumber CParser::ParseRadius(const string &input)
{
	size_t index;
	for (index = 0; index < input.size(); ++index)
	{
	...
	string radius = input.substr(index, input.length() - index - 1);

	return CLongNumber(radius);
}
...
CLongNumber CParser::GetRadius() const
{
	return m_radius;
}
